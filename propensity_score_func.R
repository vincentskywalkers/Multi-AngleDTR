...
Ctfunc <- function(mt,N){
 # C = sample(c(mt,3),N,replace = T,prob=c(1,1,1,1,1,1))
  #C = sample(c(mt,3),N,replace = T,prob=c(0.1,0.25,0.25,0.25,1,1))
  #C = sample(c(mt,3),N,replace = T,prob=c(0.5,0.5,0.5,1,1,1))
  C = sample(c(mt,3),N,replace = T,prob=c(10,5,1,1,1,1))
  #C = sample(c(mt,3),N,replace = T,prob=c(1,1,5,1,1,1))
...
Tffunc <- function(N,M,A,d,X){
  ...
  Tf = c()
  for (i in 1:N){
    Tfy[i,1] = 0.5*I(d[i,1]==A[i,1])*rbinom(1,1,prob=0.95-0.1*I(X[i,1,1] + X[i,1,3] >0)) +
      I(d[i,1]!=A[i,1])*sample(c(0.5,0.25,0),size=1,prob=c(0.6,0.15,0.25))
    for (ttt in 2:(M-2)){
      Tmp =  0.5*I(d[i,ttt]==A[i,ttt])*rbinom(1,1,prob=0.95-0.1*I(X[i,ttt,1] + X[i,ttt,3] >0)) +
        I(d[i,ttt]!=A[i,ttt])*sample(c(0.5,0.25,0),size=1,prob=c(0.6,0.15,0.25))
...
#   Tf = c()
#   for (i in 1:N){
#     Tfy[i,1] = 0.5*I(d[i,1]==A[i,1])*rbinom(1,1,prob=0.97-0.02*I(X[i,1,1] + X[i,1,3] >0)) +
#       I(d[i,1]!=A[i,1])*sample(c(0.5,0.25,0),size=1,prob=c(0.6,0.15,0.25))
#     for (ttt in 2:(M-2)){
#       Tmp =  0.5*I(d[i,ttt]==A[i,ttt])*rbinom(1,1,prob=0.97-0.02*I(X[i,ttt,1] + X[i,ttt,3] >0)) +
#         I(d[i,ttt]!=A[i,ttt])*sample(c(0.5,0.25,0),size=1,prob=c(0.6,0.15,0.25))
#       Tfy[i,ttt] = I(Tfy[i,(ttt-1)]>0.25)*Tmp
#     }
#     for (ttt in (M-1):(M)){
...
trainfunc <- function(N){
  ...
  prob_11 = pro_gen1(X[,1,]%*%probeta1,X[,1,]%*%probeta2)
  prob_12 = pro_gen2(X[,1,]%*%probeta1,X[,1,]%*%probeta2)
  prob_13 = pro_gen3(X[,1,]%*%probeta1,X[,1,]%*%probeta2)
  
  for (i in 1:N) {
    A[i,1]=sample(x=c(1,2,3),1,replace = T,prob = c(prob_11[i],prob_12[i],prob_13[i]))
  }
  
  f_11=X[,1,]%*%beta1[1:dimco]
  f_12=X[,1,]%*%beta1[(dimco+1):(2*dimco)]
  
  prob_21 = pro_gen1(X[,1,]%*%probeta1+X[,2,]%*%probeta1,X[,1,]%*%probeta2+X[,2,]%*%probeta2)
  prob_22 = pro_gen2(X[,1,]%*%probeta1+X[,2,]%*%probeta1,X[,1,]%*%probeta2+X[,2,]%*%probeta2)
  prob_23 = pro_gen3(X[,1,]%*%probeta1+X[,2,]%*%probeta1,X[,1,]%*%probeta2+X[,2,]%*%probeta2)
  
  for (i in 1:N) {
    A[i,2]=sample(x=c(1,2,3),1,replace = T,prob = c(prob_21[i],prob_22[i],prob_23[i]))
  }
  ...
  prob_31 = pro_gen1(X[,1,]%*%probeta1+X[,2,]%*%probeta1+X[,3,]%*%probeta1,X[,1,]%*%probeta2+X[,2,]%*%probeta2+X[,3,]%*%probeta2)
  prob_32 = pro_gen2(X[,1,]%*%probeta1+X[,2,]%*%probeta1+X[,3,]%*%probeta1,X[,1,]%*%probeta2+X[,2,]%*%probeta2+X[,3,]%*%probeta2)
  prob_33 = pro_gen3(X[,1,]%*%probeta1+X[,2,]%*%probeta1+X[,3,]%*%probeta1,X[,1,]%*%probeta2+X[,2,]%*%probeta2+X[,3,]%*%probeta2)
  
  for (i in 1:N) {
    A[i,3]=sample(x=c(1,2,3),1,replace = T,prob = c(prob_31[i],prob_32[i],prob_33[i]))
  }
  ...
  prob_41 = pro_gen1(X[,1,]%*%probeta1+X[,2,]%*%probeta1+X[,3,]%*%probeta1+X[,4,]%*%probeta1,X[,1,]%*%probeta2+X[,2,]%*%probeta2+X[,3,]%*%probeta2+X[,4,]%*%probeta2)
  prob_42 = pro_gen2(X[,1,]%*%probeta1+X[,2,]%*%probeta1+X[,3,]%*%probeta1+X[,4,]%*%probeta1,X[,1,]%*%probeta2+X[,2,]%*%probeta2+X[,3,]%*%probeta2+X[,4,]%*%probeta2)
  prob_43 = pro_gen3(X[,1,]%*%probeta1+X[,2,]%*%probeta1+X[,3,]%*%probeta1+X[,4,]%*%probeta1,X[,1,]%*%probeta2+X[,2,]%*%probeta2+X[,3,]%*%probeta2+X[,4,]%*%probeta2)
  
  for (i in 1:N) {
    A[i,4]=sample(x=c(1,2,3),1,replace = T,prob = c(prob_41[i],prob_42[i],prob_43[i]))
  }
  ...
  prob_51 = pro_gen1(X[,1,]%*%probeta1+X[,2,]%*%probeta1+X[,3,]%*%probeta1+X[,4,]%*%probeta1+X[,5,]%*%probeta1,X[,1,]%*%probeta2+X[,2,]%*%probeta2+X[,3,]%*%probeta2+X[,4,]%*%probeta2+X[,5,]%*%probeta2)
  prob_52 = pro_gen2(X[,1,]%*%probeta1+X[,2,]%*%probeta1+X[,3,]%*%probeta1+X[,4,]%*%probeta1+X[,5,]%*%probeta1,X[,1,]%*%probeta2+X[,2,]%*%probeta2+X[,3,]%*%probeta2+X[,4,]%*%probeta2+X[,5,]%*%probeta2)
  prob_53 = pro_gen3(X[,1,]%*%probeta1+X[,2,]%*%probeta1+X[,3,]%*%probeta1+X[,4,]%*%probeta1+X[,5,]%*%probeta1,X[,1,]%*%probeta2+X[,2,]%*%probeta2+X[,3,]%*%probeta2+X[,4,]%*%probeta2+X[,5,]%*%probeta2)
  
  for (i in 1:N) {
    A[i,5]=sample(x=c(1,2,3),1,replace = T,prob = c(prob_51[i],prob_52[i],prob_53[i]))
  }
  ...
  datainfo = list(X=X,d=d,A=A,f1=f1,f2=f2)
  return(datainfo)
...
giventimefn <- function(givent,mt,N,train){
  X = train$X
  A = train$A
  if(givent>=mt[1]&givent<mt[2]){
    phat=matrix(NA,N,1)
    promat1 = propensity.multinom.lasso(X[,1,],A[,1])
    for (i in 1:N){
      phat[i] = promat[i,A[i,1]]
    }
    beta = beta1
  }
  if(givent>=mt[2]&givent<mt[3]){
    ...
    phat2=matrix(NA,N,1)
    promat1 = propensity.multinom.lasso(X[,1,],A[,1])
    promat2 = propensity.multinom.lasso(cbind(X[,1,],X[,2,]),A[,2])
    for (i in 1:N){
      phat1[i] = promat1[i,A[i,1]]
    }
    for (i in 1:N){
      phat2[i] = promat2[i,A[i,2]]
    }
    phat = cbind(phat1,phat2)
    beta = c(beta1,beta2)
  }
  if(givent>=mt[3]&givent<mt[4]){
    ...
    promat1 = propensity.multinom.lasso(X[,1,],A[,1])
    promat2 = propensity.multinom.lasso(cbind(X[,1,],X[,2,]),A[,2])
    promat3 = propensity.multinom.lasso(cbind(X[,1,],X[,2,],X[,3,]),A[,3])
    for (i in 1:N){
      phat1[i] = promat1[i,A[i,1]]
    }
    for (i in 1:N){
    ...
    for (i in 1:N){
      phat3[i] = promat3[i,A[i,3]]
    }
    phat = cbind(phat1,phat2,phat3)
  ...
  if(givent>=mt[4]&givent<mt[5]){
    ...
    promat2 = propensity.multinom.lasso(cbind(X[,1,],X[,2,]),A[,2])
    promat3 = propensity.multinom.lasso(cbind(X[,1,],X[,2,],X[,3,]),A[,3])
    promat4 = propensity.multinom.lasso(cbind(X[,1,],X[,2,],X[,3,],X[,4,]),A[,4])
    for (i in 1:N){
      phat1[i] = promat1[i,A[i,1]]
    }
    for (i in 1:N){
    ...
    for (i in 1:N){
      phat4[i] = promat4[i,A[i,4]]
    }
    phat = cbind(phat1,phat2,phat3,phat4)
  ...
  if(givent>=mt[5]){
    ...
    promat3 = propensity.multinom.lasso(cbind(X[,1,],X[,2,],X[,3,]),A[,3])
    promat4 = propensity.multinom.lasso(cbind(X[,1,],X[,2,],X[,3,],X[,4,]),A[,4])
    promat5 = propensity.multinom.lasso(cbind(X[,1,],X[,2,],X[,3,],X[,4,],X[,5,]),A[,5])
    for (i in 1:N){
      phat1[i] = promat1[i,A[i,1]]
    }
    for (i in 1:N){
...
crossvalidate <- function(train,Kfold,lambda,b){
  ...
  for (kkk in 1:Kfold){
    ...
    giventmp = giventimefnprop(givent,mt,corss_N,corss_X,corss_A)
    phat = giventmp$phat
    ...
    C_x = max(max(corss_X[,1:Nstep,]),max(corss_A[,1:Nstep]))
    ...
    giventmp = giventimefnprop(givent,mt,validate_N,validate_X,validate_A)
    phat = giventmp$phat
    Nstep=length(phat[1,])
...
pro_gen1 <- function(a,b){
  c = 1/(1+exp(a)+exp(b))
  return(c)
}

pro_gen2 <- function(a,b){
  c = exp(a)/(1+exp(a)+exp(b))
  return(c)
}

pro_gen3 <- function(a,b){
  c = exp(b)/(1+exp(a)+exp(b))
  return(c)
}
...
propensity.multinom.lasso <- function(x, trt)
{
  ...
  gfit <- cv.glmnet(y = trt, x = x,alpha=1, family = "multinomial")
