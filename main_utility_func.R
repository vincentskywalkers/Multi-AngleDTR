...
Ctfunc <- function(mt,N){
  ...
  #C = sample(c(mt,3),N,replace = T,prob=c(0.25,0.5,0.5,1,1,1))
  return(C)
}
##----------------------------------------------------
...
#     } else if (indexstep[ii] < Nstep){
#       for (ss in 1:indexstep[ii]) {
#         phat[ii,min((ss+1),Nstep)] = 1/3
#         A_complete = A[,1:Nstep]
#         A_complete[ii,((indexstep[ii]+1):Nstep)] = d[ii,((indexstep[ii]+1):Nstep)]
#         A_complete[ii,(1:indexstep[ii])] = A[ii,(1:indexstep[ii])]
#         W[ss,ii]=prod((A_complete[ii,1:min((ss+1),Nstep)]==d[ii,1:min((ss+1),Nstep)]))/prod(phat[ii,1:min((ss+1),Nstep)])  #min(lum[1:ss,ii])
#       } 
#     }
#   }
#   return(W)
